...
fn parse_font_faces(css: &str, stylesheet_url: &Url) -> Vec<RawFontFace> {
    ...
    for block_match in block_re.captures_iter(css) {
        ...
        // Find the best font URL (prefer woff2 > woff > ttf > otf)
...
/// Result of synchronous HTML parsing — all owned data, no references to Html.
struct ParsedPage {
...
/// Parse HTML synchronously, extracting all favicon/font data into owned types.
/// The non-Send `scraper::Html` is created and dropped entirely within this
/// function, so it never appears in any async function's state machine.
fn parse_html_sync(html_text: &str, base_url: &Url) -> ParsedPage {
    ...
    // --- Extract inline @font-face + stylesheet URLs ---
    ...
    // `document` (Html) is dropped here at end of function scope
    ParsedPage {
        favicons,
        favicon_ico_url,
        inline_faces,
...
/// Async: fetch stylesheets and parse @font-face from them.
...
/// Async: check if /favicon.ico exists.
...
pub async fn scan_website(url: String) -> Result<ScanResult, String> {
    ...
    // Run HTML parsing on a blocking thread so the non-Send `scraper::Html`
    // type never pollutes the async function's state machine.
    ...
    // --- Async work: fetch favicon.ico and stylesheets ---
